...
import (
	...
	"github.com/acentswap/aswap/v3/common"
	"github.com/acentswap/aswap/v3/common/hexutil"
	"github.com/acentswap/aswap/v3/log"
	"github.com/acentswap/aswap/v3/params"
	"github.com/acentswap/aswap/v3/router"
	"github.com/acentswap/aswap/v3/tokens"
	"github.com/acentswap/aswap/v3/tokens/eth/abicoder"
)
...
var (
	...
	// acentswapIn(bytes32 txs, address token, address to, uint amount, uint fromChainID)
	acentswapInFuncHash = common.FromHex("0x825bb13c")
	// acentswapInUnderlying(bytes32 txs, address token, address to, uint amount, uint fromChainID)
	acentswapInUnderlyingFuncHash = common.FromHex("0x3f88de89")
	// acentswapInAuto(bytes32 txs, address token, address to, uint amount, uint fromChainID)
	acentswapInAutoFuncHash = common.FromHex("0x0175b1c4")
	// acentswapInExactTokensForTokens(bytes32 txs, uint amountIn, uint amountOutMin, address[] path, address to, uint deadline, uint fromChainID)
	acentswapInExactTokensForTokensFuncHash = common.FromHex("0x2fc1e728")
	// acentswapInExactTokensForNative(bytes32 txs, uint amountIn, uint amountOutMin, address[] path, address to, uint deadline, uint fromChainID)
	acentswapInExactTokensForNativeFuncHash = common.FromHex("0x52a397d5")
)
...
func (b *Bridge) buildERC20SwapoutTxInput(args *tokens.BuildTxArgs, multichainToken string) (err error) {
	...
	if erc20SwapInfo.ForUnderlying {
		funcHash = acentswapInUnderlyingFuncHash
	} else {
		funcHash = acentswapInAutoFuncHash // old:acentswapInFuncHash
	}
...
func (b *Bridge) buildERC20SwapTradeTxInput(args *tokens.BuildTxArgs, multichainToken string) (err error) {
	...
	if erc20SwapInfo.ForNative {
		funcHash = acentswapInExactTokensForNativeFuncHash
	} else {
		funcHash = acentswapInExactTokensForTokensFuncHash
	}
