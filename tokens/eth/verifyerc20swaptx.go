...
import (
	...
	"github.com/acentswap/aswap/v3/types"
)

// router contract's log topics
var (
	// LogacentswapOut(address token, address from, address to, uint amount, uint fromChainID, uint toChainID);
	LogacentswapOutTopic = common.FromHex("0x97116cf6cd4f6412bb47914d6db18da9e16ab2142f543b86e207c24fbd16b23a")
	// LogacentswapTradeTokensForTokens(address[] path, address from, address to, uint amountIn, uint amountOutMin, uint fromChainID, uint toChainID);
	LogacentswapTradeTokensForTokensTopic = common.FromHex("0xfea6abdf4fd32f20966dff7619354cd82cd43dc78a3bee479f04c74dbfc585b3")
	// LogacentswapTradeTokensForNative(address[] path, address from, address to, uint amountIn, uint amountOutMin, uint fromChainID, uint toChainID);
	LogacentswapTradeTokensForNativeTopic = common.FromHex("0x278277e0209c347189add7bd92411973b5f6b8644f7ac62ea1be984ce993f8f4")
)
...
func (b *Bridge) verifyERC20SwapTxLog(swapInfo *tokens.SwapTxInfo, rlog *types.RPCLog) (err error) {
	...
	switch {
	case bytes.Equal(logTopic, LogacentswapOutTopic):
		err = b.parseERC20SwapoutTxLog(swapInfo, rlog)
	case bytes.Equal(logTopic, LogacentswapTradeTokensForTokensTopic):
		err = b.parseERC20SwapTradeTxLog(swapInfo, rlog, false)
	case bytes.Equal(logTopic, LogacentswapTradeTokensForNativeTopic):
